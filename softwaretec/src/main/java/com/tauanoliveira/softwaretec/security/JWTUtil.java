...
public class JWTUtil {
    ...
    public boolean tokenValido(String token) {
        Claims claims = getClaims(token);//armazena reinvidicações do token, armazena o token e o usuario dela
        if(claims != null) {
    ...
    public String getUsername(String token) {
        ...
        if(claims != null) {
            return claims.getSubject();//nome de usuario dentro do token
        }    
        return null;
    }
